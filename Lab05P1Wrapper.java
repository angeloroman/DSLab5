...
public class Lab05P1Wrapper {
	...
	public static class SinglyLinkedList<E> implements List<E> {
      ...
      Node<E> reversed = header.getNext();
      Node<E> current = header.getNext().getNext();
      reversed.setNext(null);
      E lastValue = header.getNext().getElement();

      while (current != null){
        Node<E> holder = current;
        current = current.getNext();
        holder.setNext(reversed);
        reversed = holder;
        lastValue = holder.getElement();
      }
      Node<E> firstNode = new Node(lastValue, reversed);
      header = firstNode;
    }
	}
}